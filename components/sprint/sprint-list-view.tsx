...
function TaskRow({
  ...
  React.useEffect(() => {
    setLocalDueDate(task.dueDateStart ?? null);
  }, [task.dueDateStart]);
    ...
    setMoveSprints(
      "error" in sprintsRes
        ...
        : sprintsRes.sprints.filter(
            (s) => s.status !== "CLOSED" && s.id !== sprintId
          )
    );
  ...
  async function handleToggleAssignee(userId: string | null) {
    ...
    if (isAssigned) {
      await removeAssignee(workspaceId, spaceId, task.listId, task.id, userId);
    } else {
      await addAssignee(workspaceId, spaceId, task.listId, task.id, userId);
    }
    router.refresh();
  }
  ...
  async function handleSetDueDate(date: Date | null) {
    ...
    setDateOpen(false);
    const res = await updateTask(workspaceId, spaceId, task.listId, task.id, {
      dueDateStart: date,
      dueDateEnd: date,
    });
    if ("error" in res) {
      ...
      >
        ...
        <div className="flex flex-1 items-center gap-2.5 min-w-0 py-1.5 pr-4 pl-1">
          <span className="text-2xs text-gray-400 font-mono shrink-0 select-none flex items-center gap-1">
            ...
            {localPersonalPin && (
              <PushPinIcon
                className="size-2.5 text-primary shrink-0"
                weight="fill"
              />
            )}
          </span>
          <span className="text-[13px] font-medium text-gray-800 truncate group-hover/row:text-primary transition-colors">
            {task.title}
          </span>
        ...
        {/* Assignee */}
        <div
          className="w-36 shrink-0 self-stretch flex items-center px-2"
                  ...
                  {task.assignees.length > 0 ? (
                    <div className="flex -space-x-1.5">
                      {task.assignees.slice(0, 3).map((a) => (
                        <Avatar
                          className="size-6 shrink-0 border border-white shadow-sm"
                          key={a.userId}
                        >
                          {a.image && (
                            <AvatarImage alt={a.name} src={a.image} />
                          )}
                      ...
                      {task.assignees.length > 3 && (
                        <div className="flex size-6 items-center justify-center rounded-full border border-white bg-gray-100 text-2xs text-gray-500 font-bold shadow-sm">
                          +{task.assignees.length - 3}
                        </div>
                      )}
                    </div>
            ...
            {canEdit && (
              ...
              >
                ...
                >
                  ...
                  <div className="h-px bg-border my-1" />
                  <button
                    className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-xs font-semibold hover:bg-accent cursor-pointer"
                    onClick={async (e) => {
                      e.stopPropagation();
                      await archiveTask(
                        workspaceId,
        ...
        {!collapsed && (
          <div>
            ...
            <div className="flex items-center">
              ...
              <div className="flex-1 py-2 pr-4 text-2xs font-bold text-gray-400 uppercase tracking-wider">
                Name
              </div>
              <div className="w-36 shrink-0 py-2 px-4 text-2xs font-bold text-gray-400 uppercase tracking-wider">
                Assignee
              </div>
...
function SprintBoardCardContent({
  ...
  return (
    ...
    >
      ...
      >
        <p className="text-[13px] font-medium text-gray-800 leading-snug select-none line-clamp-2">
          {task.title}
        </p>
        {task.tags.length > 0 && (
        ...
        <div className="mt-2 flex items-center justify-between gap-2">
          <span className="font-mono text-2xs text-gray-400 shrink-0">
            #{task.seqNumber}
          </span>
...
function SprintBoardColumn({
  ...
  return (
    ...
    >
      ...
      {/* Droppable task list */}
      <SortableContext
        items={draggableTasks.map((t) => t.id)}
...
export function SprintListView({
  ...
  function onDragOver({ active, over }: DragOverEvent) {
    ...
    if (!activeTask?.listId) {
      return; // sprint-only tasks can't move
    }
    const overStatus =
      effectiveStatuses.find((s) => s.id === overId)?.id ??
      boardTasks.find((t) => t.id === overId)?.statusId;
    if (!overStatus || overStatus === activeTask.statusId) {
      return;
    }
    setBoardTasks((prev) =>
    ...
    () =>
      tasks.filter((t) => {
        ...
        if (assigneeFilter.length) {
          ...
          const matchUser =
            userIds.length > 0 &&
            assigneeIds.some((id) => userIds.includes(id));
  ...
  const tasksByStatus = React.useMemo(() => {
    ...
    for (const t of filteredTasks) {
      ...
      if (group) {
        ...
        map.get(effectiveStatuses[0]?.id ?? "")?.push(t);
      }
    }
    return map;
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Popover>
            ...
            <PopoverContent align="start" className="w-64 p-3 space-y-4">
              {/* Status filter */}
              <div>
                <p className="mb-1.5 text-2xs font-bold text-gray-400 uppercase tracking-wide">
                  Status
                </p>
                <div className="flex flex-col gap-1 max-h-32 overflow-y-auto">
                  {effectiveStatuses.map((s) => (
                    <label
                      className="flex items-center gap-2 text-xs text-gray-700 cursor-pointer py-0.5 hover:bg-gray-50 rounded"
                      key={s.id}
                    >
                      <input
