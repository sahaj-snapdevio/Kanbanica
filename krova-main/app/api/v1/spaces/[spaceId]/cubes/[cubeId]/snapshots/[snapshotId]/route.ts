...
import { deleteSnapshotV1Action } from "@/lib/cube-actions/snapshots";
...
export async function DELETE(
  request: NextRequest,
  {
    ...
    params: Promise<{ spaceId: string; cubeId: string; snapshotId: string }>;
  }
) {
  try {
    ...
    const { spaceId, cubeId, snapshotId } = await params;
    ...
    const result = await deleteSnapshotV1Action({
    ...
    console.error("[v1/snapshots/[snapshotId]] DELETE request failed:", error);
