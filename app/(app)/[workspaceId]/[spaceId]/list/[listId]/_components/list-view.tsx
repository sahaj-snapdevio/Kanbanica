...
function TaskRow({
  ...
  React.useEffect(() => {
    setLocalPersonalPin(isPersonallyPinned ?? false);
  }, [isPersonallyPinned]);
  ...
  async function loadMoveData() {
    ...
    setMoveSprints(
      "error" in sprintsRes
        ? []
        : sprintsRes.sprints.filter((s) => s.status !== "CLOSED")
    );
  ...
  async function handleSetDueDate(date: Date | null) {
    ...
    setDateOpen(false);
    const res = await updateTask(workspaceId, spaceId, listId, task.id, {
      dueDateStart: date,
      dueDateEnd: date,
    });
    if ("error" in res) {
            ...
            {canEdit && (
              ...
              >
                ...
                >
                  ...
                  >
                    <ArchiveIcon className="size-3.5 text-muted-foreground shrink-0" />{" "}
                    Archive
                  </button>
                </PopoverContent>
              </Popover>
    ...
    <>
      ...
      >
                  ...
                  <PopoverContent align="start" className="w-64 p-3 space-y-4">
                    ...
                    <div>
                      ...
                      <div className="flex flex-col gap-1 max-h-32 overflow-y-auto">
                        {statuses.map((s) => (
                          <label
                            className="flex items-center gap-2 text-xs text-foreground cursor-pointer py-0.5 hover:bg-accent/30 rounded"
                            key={s.id}
                          >
                            <input
                    ...
                    <div>
                      ...
                      <div className="flex flex-col gap-1">
                        {["URGENT", "HIGH", "MEDIUM", "LOW", "NONE"].map(
                          (p) => (
                            <label
                              className="flex items-center gap-2 text-xs text-foreground cursor-pointer py-0.5 hover:bg-accent/30 rounded"
                              key={p}
        ...
        <DragOverlay>
          {activeTask && (
            <div
              className="flex items-center bg-card border border-border rounded-xl shadow-lg px-4 py-2 w-full max-w-lg cursor-grabbing text-sm border-l-[3px]"
              style={{
                borderLeftColor:
                  statuses.find((s) => s.id === activeTask.statusId)?.color ||
                  "#6B7280",
              }}
            >
              <div className="flex size-4 items-center justify-center rounded border border-border mr-3">
                {selectedIds.has(activeTask.id) && (
                  <CheckIcon className="size-2.5 text-primary" weight="bold" />
                )}
              </div>
              <span className="text-2xs text-muted-foreground font-mono mr-2">
                #{activeTask.seqNumber}
              </span>
