...
import { task, comment, space } from "@/db/schema";
...
export async function GET() {
  ...
  const [tasksPerDay, commentsPerDay, spacesPerDay, totals] = await Promise.all([
    ...
    Promise.all([
      db.select({ count: count() }).from(task).then((r) => r[0].count),
      db.select({ count: count() }).from(comment).then((r) => r[0].count),
      db.select({ count: count() }).from(space).then((r) => r[0].count),
    ]),
  ]);
