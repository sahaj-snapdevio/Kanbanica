...
import { user, workspace, task, supportTicket, auditLogs } from "@/db/schema";
...
export async function GET() {
  ...
  const [[totalUsersRow], [totalWorkspacesRow], [totalTasksRow], [openTicketsRow], [newTodayRow], [newMonthRow], recentActivity] =
    await Promise.all([
      ...
      db.select({ count: count() }).from(user).where(gte(user.createdAt, todayUTC)),
      db.select({ count: count() }).from(user).where(gte(user.createdAt, monthStartUTC)),
      db
        .select({
          id: auditLogs.id,
