...
export async function GET(req: NextRequest) {
  ...
  if (category) conditions.push(eq(helpArticle.category, category));
  if (search) conditions.push(ilike(helpArticle.title, `%${search}%`));
  ...
  const articles = await db
    ...
    .orderBy(asc(helpArticle.category), asc(helpArticle.orderIndex), asc(helpArticle.title));
